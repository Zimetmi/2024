document.addEventListener('DOMContentLoaded', function () {
    ...
    function createSelect(id, dataColumn, dataRow, placeholder) {
        ...
        select.addEventListener('input', function () {
            saveData(select.value, dataColumn, dataRow, 'brickusDay1');
        });
...
function createCheckbox(id, dataColumn, dataRow, initialValue) {
    ...
    checkbox.addEventListener('change', function () {
        if (checkbox.checked) {
            saveData('Номинант', dataColumn, dataRow, 'brickusDay1');
        } else {
            saveData('', dataColumn, dataRow, 'brickusDay1');
        }
    });
...
function createInputFields(container, rowId, placeholders, options = []) {
    ...
    textarea.addEventListener('input', function () {
        saveData(textarea.value, 'G', rowId, 'brickusDay1');
    });
 ...
 function createParticipantPanel(participant, placeholders) {
    ...
    async function fetchDataWithCache(sheetName = 'brickusDay1', includeParticipants = false) {
    ...
    async function renderData(sheetName = 'brickusDay1') {
...
async function saveData(value, column, row, sheetName = 'brickusDay1') {
...
function attachInputListeners() {
    ...
    textareas.forEach(textarea => {
        textarea.addEventListener('change', function () {
            saveData(this.value, this.getAttribute('data-column'), this.getAttribute('data-row'), 'brickusDay1');
        });
    });
}
