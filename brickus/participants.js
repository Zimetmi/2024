document.addEventListener('DOMContentLoaded', function () {
    ...
    function createSelect(id, dataColumn, dataRow, placeholder) {
        ...
        select.addEventListener('input', function () {
            saveData(select.value, dataColumn, dataRow, 'MakoDay2');
        });
...
function createCheckbox(id, dataColumn, dataRow, initialValue) {
    ...
    checkbox.addEventListener('change', function () {
        if (checkbox.checked) {
            saveData('Номинант', dataColumn, dataRow, 'MakoDay2');
        } else {
            saveData('', dataColumn, dataRow, 'MakoDay2');
        }
    });
...
function createInputFields(container, rowId, placeholders, options = []) {
    ...
    textarea.addEventListener('input', function () {
        saveData(textarea.value, 'G', rowId, 'MakoDay2');
    });
 ...
 function createParticipantPanel(participant, placeholders) {
    ...
    async function fetchDataWithCache(sheetName = 'MakoDay2', includeParticipants = false) {
    ...
    async function renderData(sheetName = 'MakoDay2') {
...
async function saveData(value, column, row, sheetName = 'MakoDay2') {
...
function attachInputListeners() {
    ...
    textareas.forEach(textarea => {
        textarea.addEventListener('change', function () {
            saveData(this.value, this.getAttribute('data-column'), this.getAttribute('data-row'), 'MakoDay2');
        });
    });
}
